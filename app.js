// ─────────────── DATA ───────────────
const menuItems = [
  { id:1, name:'Pepperoni Pizza', price:9.99, cat:'Pizza', menuGroup:'Mains', stock:25, active:true, is86:false, emoji:'🍕', type:'Item' },
  { id:2, name:'BBQ Chicken Wings', price:12.49, cat:'Wings', menuGroup:'Starters', stock:0, active:true, is86:true, emoji:'🍗', type:'Item' },
  { id:3, name:'Caesar Salad', price:8.99, cat:'Salads', menuGroup:'Starters', stock:18, active:true, is86:false, emoji:'🥗', type:'Item' },
  { id:4, name:'Margherita Pizza', price:8.49, cat:'Pizza', menuGroup:'Mains', stock:30, active:false, is86:false, emoji:'🍕', type:'Item' },
  { id:5, name:'Fettuccine Alfredo', price:11.99, cat:'Pasta', menuGroup:'Mains', stock:12, active:true, is86:false, emoji:'🍝', type:'Item' },
  { id:6, name:'Tiramisu', price:6.49, cat:'Desserts', menuGroup:'Desserts', stock:8, active:true, is86:false, emoji:'🍰', type:'Item' },
  { id:7, name:'Sparkling Water', price:2.99, cat:'Beverages', menuGroup:'Drinks', stock:50, active:true, is86:false, emoji:'💧', type:'Item' },
  { id:8, name:'Corn Pizza', price:9.49, cat:'Pizza', menuGroup:'Mains', stock:20, active:true, is86:false, emoji:'🍕', type:'Item' },
  { id:9, name:'Extra Cheese', price:1.50, cat:'Pizza', menuGroup:'Add-ons', stock:100, active:true, is86:false, emoji:'🧀', type:'Modifier' },
  { id:10, name:'Ranch Dip', price:0.50, cat:'Wings', menuGroup:'Add-ons', stock:50, active:true, is86:false, emoji:'🥣', type:'Modifier' }
];
...
function openItemEditor(id) {
  ...
  // ── TAB 2: WEEKLY SALES (no hero tile) ──
  const salesHtml = `

    <!-- KPI strip -->
    <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:8px;margin-bottom:12px">
      <div style="background:${cardBg};border:1px solid ${bdr};border-radius:14px;padding:12px;text-align:center">
        <div style="font-size:18px;font-weight:900;color:var(--accent)">${totalSold}</div>
        <div style="font-size:9px;font-weight:700;text-transform:uppercase;letter-spacing:.06em;color:${textMuted};margin-top:3px">${isModifier?'Uses':'Sold'}</div>
      </div>
      <div style="background:${cardBg};border:1px solid ${bdr};border-radius:14px;padding:12px;text-align:center">
        <div style="font-size:18px;font-weight:900;color:var(--green)">$${weekRevenue}</div>
        <div style="font-size:9px;font-weight:700;text-transform:uppercase;letter-spacing:.06em;color:${textMuted};margin-top:3px">Revenue</div>
      </div>
      <div style="background:${cardBg};border:1px solid ${bdr};border-radius:14px;padding:12px;text-align:center">
        <div style="font-size:18px;font-weight:900;color:${textMain}">${peakDay}</div>
        <div style="font-size:9px;font-weight:700;text-transform:uppercase;letter-spacing:.06em;color:${textMuted};margin-top:3px">Peak Day</div>
      </div>
    </div>
    ...
    <div style="background:${cardBg};border:1px solid ${bdr};border-radius:16px;overflow:visible;margin-bottom:12px">
      <div style="display:flex;align-items:flex-start;justify-content:space-between;padding:16px 16px 0">
        <div>
          <div style="font-size:13px;font-weight:800;color:${textMain}">${isModifier ? 'Modifier Usage' : 'Weekly Sales'}</div>
          <div style="font-size:11px;color:${textMuted};margin-top:2px">Last 7 days · hover bars for details</div>
        </div>
        <div style="text-align:right">
          <span style="font-size:11px;font-weight:700;color:${trendUp?'var(--green)':'var(--red)'}">
            ${trendUp?'▲':'▼'} ${Math.abs(trendPct)}%
          </span>
          <div style="font-size:10px;color:${textMuted};margin-top:1px">vs last week</div>
        </div>
      </div>
    ...
    <!-- Recent Orders -->
    <div style="background:${cardBg};border:1px solid ${bdr};border-radius:16px;overflow:hidden">
      <div style="display:flex;align-items:center;justify-content:space-between;padding:14px 16px 10px">
        <div>
          <div style="font-size:13px;font-weight:800;color:${textMain}">Recent Orders</div>
          <div style="font-size:11px;color:${textMuted};margin-top:2px">${isModifier ? 'Orders using this modifier' : 'Orders containing this item'}</div>
        </div>
        ${recentOrders.length > 0 ? `<span style="font-size:10px;font-weight:700;padding:3px 9px;border-radius:20px;background:rgba(124,58,237,.1);color:var(--accent)">${recentOrders.length}</span>` : ''}
      </div>
      <div style="height:1px;background:${bdr}"></div>
      ${orderRowsHtml(recentOrders)}
      ${historyOrders.length > 10 ? `
        <div style="height:1px;background:${bdr}"></div>
        <div style="padding:12px 16px;text-align:center;font-size:11px;font-weight:700;color:var(--accent);cursor:pointer">View ${historyOrders.length - 10} more orders →</div>` : ''}
    </div>`;
...
function saveItem() {
  ...
  if (priceErr) priceErr.style.display = 'none';
  item.price = parsedPrice;
  item.active = document.getElementById('editActive').checked;
  item.is86 = document.getElementById('edit86').checked;
  item.stock = parseInt(document.getElementById('editQty').value) || item.stock;
...
const devicesData = [
  { id:1, name:'Front Counter POS', type:'pos',      status:'online',   deviceId:'POS-FC-001-A2B3',   location:'Front Counter', classification:'POS',      ipAddress:'192.168.1.10', defaultTerminal:'Terminal 1' },
  { id:2, name:'Bar POS',           type:'pos',      status:'offline',  deviceId:'POS-BAR-002-C4D5',  location:'Bar Station',   classification:'POS',      ipAddress:'192.168.1.11', defaultTerminal:'Terminal 2' },
  { id:3, name:'Server Handheld 1', type:'handheld', status:'online',   deviceId:'HH-S1-001-E6F7',    location:'Main Floor',    classification:'Handheld', ipAddress:'192.168.1.20', defaultTerminal:'Terminal 1' },
  { id:4, name:'Server Handheld 2', type:'handheld', status:'inactive', deviceId:'HH-S2-002-G8H9',    location:'Patio',         classification:'Handheld', ipAddress:'192.168.1.21', defaultTerminal:'Terminal 1' },
  { id:5, name:'Drive-Thru Kiosk',  type:'kiosk',   status:'online',   deviceId:'KIOSK-DT-001-I0J1', location:'Drive-Thru',    classification:'Kiosk',    ipAddress:'192.168.1.30', defaultTerminal:'Terminal 1' },
  { id:6, name:'Lobby Kiosk',       type:'kiosk',   status:'inactive', deviceId:'KIOSK-LB-002-K2L3', location:'Lobby',         classification:'Kiosk',    ipAddress:'192.168.1.31', defaultTerminal:'Terminal 2' },
];
...
function renderDeviceList() {
  ...
  list.forEach(function(d) {
    ...
    html += "<div onclick=\"openDeviceDetail(" + d.id + ")\" style=\"background:var(--surface);border:1px solid var(--border);border-left:3px solid " + borderCol + ";border-radius:14px;padding:14px;margin-bottom:10px;cursor:pointer\">"
      ...
      + "<div><div style=\"font-size:13px;font-weight:800;color:var(--text)\">" + d.name + "</div>"
...
function activateNewDevice(typeId) {
  ...
  var newDev = { id:devicesData.length+1, name:name.trim(), type:typeId, status:'online', deviceId:typeId.toUpperCase()+'-'+Math.random().toString(36).slice(2,8).toUpperCase(), location:loc };
